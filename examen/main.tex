...
\begin{figure}
  ...
  %\begin{minipage}[b][][b]{0.33\linewidth}
  ...
  \includegraphics[width=0.45\linewidth]{q4_trajectory_el45_dev35.png}
  %\end{minipage}
  %$\phi = 20\degree$, $c = 10\degree$ &
  %$\phi = 45\degree$, $c = 10\degree$ &
  %$\phi = 45\degree$, $c = 35\degree$ &
  %$\phi = 75\degree$, $c = 20\degree$
  %\end{tabular}
...
This property can be used for rendering lambertian materials efficiently, as demonstrated by~\cite{Ramamoorthi2001a}. Suppose you have computed the spherical harmonics transform on an environment map $\hat{\B{L}}$ and a lambertian reflectance function $\hat{f}$ of a given surface normal. To compute the irradiance $\B{E}$ of this surface, it is possible to perform the convolution---a dot product in the frequency domain---between the environment map and the reflectance function as follows:
\begin{align}
...
\qquad,
\end{align}
for all the computed degrees $n$ and orders $m$.
