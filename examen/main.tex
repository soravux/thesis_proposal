...
Such an algorithm is presented in~\cite{Granados2010a}. The goal is to find the weights to fusion the images together in order to estimate as best as possible the irradiance of each pixel. Basically, this algorithm is as follows:
\begin{enumerate}
  \item{Acquire dark frames for all exposures values $e_i$;}
  \item{Assume constant variances $\hat{\sigma}_{X_i}^2{}^{(0)}$}
  \item{Compute the maximum likelihood estimate $\hat{\mu}_X^{(i)}$ for each pixel irradiance taking into account $\hat{\sigma}_{X_i}^2{}^{(i-1)}$;}
  \item{Estimate the pixel variances $\hat{\sigma}_{X_i}^2{}^{(i)}$ taking into account $\hat{\mu}^{(i-1)}$;}
  \item{Repeat the two previous steps until convergence;}
  \item{Smooth the final maximum likelihood estimate $\hat{\mu}_X$ according to the pixel variances $\hat{\sigma}_{X_i}^2$}
\end{enumerate}

The optimal weighting function $w_{\mathrm{opt}}\left(v_i\right)$ of every pixel $v_i$ is the reciprocal of the pixel variances $\frac{1}{\hat{\sigma}_{X_i}^2}$.

This algorithm is based on the idea that an image of given exposure $i$ has an irradiance $X_i$ related to the according to the image taken $V_i$ dark frame $B_i$, the exposure time $t_i$ and the camera gains $g$ (global) and $a$ (non-uniform, pixel-dependent) as such:
\begin{equation}
\label{q7:eqirr}
X_i \approx \frac{V_i - B_i}{t_i \cdot g \cdot a}
\quad.
\end{equation}

Two types of noise is taken into account in this analysis: the thermal noise and spatial noise. In the first category fall the Photon Shot Noise, accounting the uncertainty of photons arrival and amounting to $\sqrt{\frac{X}{t_i}}$, the Dark Current Shot Noise, accounting for electron movement not caused by incoming photons, and the readout noise, explaining the noise and imprecision in the electronic circuit producing the output digital values. The second category, spatial noise, regroup Photo-Response Non-Uniformity, accounting for variability in gain over the various pixels, and the Dark Current Non-Uniformity, accounting for dark current variability across the sensor surface due to temperature differences.

The output digital value $V$ of a pixel $j$ on image $i$ is related to the irradiance it received $X_j$ by the exposure time $t_i$, the dark current $D_j$, the readout noise $N_R$ and sensor gains $g$ and $a_j$ as such:
\begin{equation}
V_{ij} = \left[ g \cdot \left( t_i \left( a_j X_j + D_j \right ) + N_R\right) \right] \quad,
\end{equation}
...
\begin{equation}
\sigma_{X_{ij}}^2 = \frac{\sigma_{V_{ij}}^2 + \sigma_{B_{ij}}^2}{t_i^2 g^2 a_j^2} \quad.
\end{equation}
...
\begin{equation}
\sigma_{\mu_{X_j}}^2 = \frac{1}{\sum_{i \in S_j} \frac{t_i^2 g^2 a_j^2}{g^2 t_i \left( a_j \mu_{X_j} + 2 \mu_{D_j} \right) + 2 \sigma_R^2}} \quad,
\end{equation}
when taking into account the set of images $S_j$ at pixel $j$.
