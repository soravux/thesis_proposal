...
\begin{figure}
  ...
  \caption[Experiment 2]
   {Angular error in degrees (z-axis) and reciprocal condition number (color) in function of $\theta_{\text{azimuth}} \in \left[0, 180\degree\right]$ (y-axis) and $r \in \left[0, 70\degree\right]$ (x-axis). Some example angles for $\theta_{\text{azimuth}}$ are shown in fig.~\ref{q3:angle_setup}. Both figures are two different point of views of the same function. Note how the region around $r = 30\degree$ and $\epsilon = 10\degree$ have a relatively good conditioning (shown in green), but the result is much worse than regions with a lower measurement error $\lvert \epsilon \rvert$.}
  \label{q3:exp2}
\end{figure}
...
\begin{figure}
  ...
  \caption[Experiment 2 on a small lighting angle]
   {Same experiment as fig.~\ref{q3:exp2}, but with a lighting angle $r = 5\degree$. The missing section in the surface is when the error is $\epsilon_r = -5\degree$, meaning that the error-tainted measure positioned all three light sources at the exact same location ($r = 0\degree$).}
  \label{q3:exp2a}
\end{figure}
